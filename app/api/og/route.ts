...
export async function GET(req: Request) {
  try {
    ...
    // Determine image to use as hero (admin-specified OG image, splash or hero)
    ...
    const imageUrl = imagePath ? (imagePath.startsWith('http') ? imagePath : `${baseUrl}/${String(imagePath).replace(/^\/*/, '')}`) : null
